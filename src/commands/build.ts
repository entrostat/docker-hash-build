...
export default class Build extends BaseCommand {
  ...
  static examples = [
    ...
    `<%= config.bin %> <%= command.id %> . --image-name=kerren/docker-hash-build --buildx-additional-build-flags="--cache-to type=gha,mode=max,scope=$GITHUB_REF_NAME-image-name --cache-from type=gha,scope=$GITHUB_REF_NAME-image-name"`,
  ];
